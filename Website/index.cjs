...
app.post("/setobjectdetection", function (req, res) {
  console.log("HTTP POST Request");

  const latitude = req.body.latitude;
  const longitude = req.body.longitude;
  const knife = req.body.knife;
  const guns = req.body.guns;
  const sharpObjects = req.body.sharpobjects;
  const explosive = req.body.explosive;
  const url = req.body.url;

  const id = Math.random().toString(36).substr(2, 9);
  const referencePath = "/objectdetection/" + id + "/";
  ...
  userReference.set({
      ...
      if (error) {
          ...
          res.json({ success: true, message: 'Location and information received and saved successfully' });
      }
  });
});

app.get("/getobjectdetection", function (req, res) {
  const id = req.params.id;

  const referencePath = "/objectdetection/";
  const userReference = firebase.database().ref(referencePath);
